...
@section('title','Palm Reading Online')
@section('content')
...
<style>
    ...
    /* Custom style untuk input file */
    input[type="file"]::file-selector-button {
        background: #3d3020; color: #d4af37; border: 1px solid #d4af37;
        padding: 5px 15px; border-radius: 8px; cursor: pointer; margin-right: 15px;
    }

    .form-check {
        background: rgba(255, 255, 255, 0.02); padding: 15px 15px 15px 40px;
        border-radius: 12px; border: 1px solid rgba(212, 175, 55, 0.1); margin-bottom: 10px;
    }
    ...
    <div class="container">
        <div class="booking-card-mystic wow fadeInUp">
            <form action="/booking/store" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label class="form-label">Upload Foto Telapak Tangan (Jelas & Terang)</label>
                    ...
                    @foreach($addons as $addon)
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="addon-{{ $addon->id }}" style="color: #ccc;">
                                {{ $addon->name }} <span style="color: #d4af37;">(+Rp {{ number_format($addon->price,0,',','.') }})</span>
                            </label>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="text-center mt-5" style="border-top: 1px solid #3d3020; padding-top: 25px;">
                    ...
                    <button type="submit" class="btn-pink-mystic">REVEAL MY DESTINY</button>
                </div>
            </form>
        </div>
    </div>
